...
def urls_show():
    ...
    return render_template('sites.html', data=db.get_urls_with_checks(connection))
...
def post_url_for_checking():
    ...
    if not is_url_exists:
        ...
        url_info = db.get_url_by_name(connection, parsed_url)
        flash('Страница уже существует', 'success')
        return redirect(url_for('get_url_detail', url_id=url_info.id))
...
def get_url_detail(url_id):
    ...
    return render_template('url_detail.html', url_info=url_info, checks_list=checks_list)
